...
void MtgaMatch::onMatchStateDiff(MatchStateDiff matchStateDiff)
{
  ...
  for (int objectId : idsZoneChanged.keys())
  {
    ...
    ZoneTransferType zoneTransferType = getZoneTransferType(objectId, zoneSrc, zoneDst, zoneTransferCategory);
...
void MtgaMatch::notifyCardZoneChange(int objectId, int oldObjectId, MatchZone zoneSrc, MatchZone zoneDst,
                                     ZoneTransferType zoneTransferType)
{
  ...
  switch (zoneTransferType)
  {
    ...
    default:
    {
      LOGD(QString("*** %1 move %2 from %3 to %4")
               .arg(ownerIdenfitier)
               .arg(cardName)
               .arg(zoneSrc.name())
               .arg(zoneDst.name()));
...
{
  int ownerId = zoneSrc.type() == ZoneType_STACK ? stackOwnerTrack[objectId] : zoneSrc.ownerSeatId();
