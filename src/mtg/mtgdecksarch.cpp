...
void MtgDecksArch::loadDecksArchFromFile()
{
  ...
  LOGI(QString("Decks architectures loaded with %1 archtectures").arg(jsonDecksArchitecture.count()));
}
...
void MtgDecksArch::downloadDecksArch()
{
  ...
  connect(reply, &QNetworkReply::finished, this, &MtgDecksArch::downloadDecksArchOnFinish);
...
QString MtgDecksArch::findDeckArchitecture(QMap<Card*, int> cards)
{
  ...
  if (diff < DIFF_SIMILARITY_THRESHOLD)
  {
    ...
    if (LOG_DECK_ARCH_CALC)
    {
      LOGD(QString("--- First Arch --- %1: %2 (With lands)").arg(archFirst.name).arg(archFirstSimilarityWithLands));
    }
    double archSecondSimilarityWithLands = getCardsSimilarityForDeckArch(cards, archSecond.cards);
    if (LOG_DECK_ARCH_CALC)
    {
      LOGD(
          QString("--- Second Arch --- %1: %2 (With lands)\n").arg(archSecond.name).arg(archSecondSimilarityWithLands));
    }
    DeckArch arch = archSecondSimilarityWithLands > archFirstSimilarityWithLands ? archSecond : archFirst;
    if (archsWithDeckColors.size() == 2)
    {
      return arch.name;
    }
    double archSimilarityWithLands = archSecondSimilarityWithLands > archFirstSimilarityWithLands ?
                                         archSecondSimilarityWithLands :
                                         archFirstSimilarityWithLands;
    ...
    if (diff2 < DIFF_SIMILARITY_THRESHOLD)
    {
      ...
      if (LOG_DECK_ARCH_CALC)
      {
        LOGD(QString("--- Third Arch --- %1: %2 (With lands)").arg(archThird.name).arg(archThirdSimilarityWithLands));
      }
      return archThirdSimilarityWithLands > archSimilarityWithLands ? archThird.name : arch.name;
    }
    return arch.name;
  }
  else
...
double MtgDecksArch::getCardsSimilarityForDeckArch(QMap<Card*, int> cards, QMap<int, double> archCards)
{
  ...
  for (Card* card : cards.keys())
  {
    ...
    double cardQtdAveragePercentValue =
        (archCardQtdAverage < cardQtd) ? archCardQtdAverage / cardQtd : cardQtd / archCardQtdAverage;
    ...
    if (LOG_DECK_ARCH_CALC)
    {
      LOGD(QString("%1: %2 of %3 = %4% * %5(Rel) * %6(Avg) = %7")
               .arg(card->name)
               .arg(cardQtd)
               .arg(archCardQtdAverage)
               .arg(cardQtdAveragePercentValue)
               .arg(cardRelevance)
               .arg(archCardQtdAverage)
               .arg(cardValue));
    }
                                                    ...
                                                    QPair<DeckArch, double> archSecond)
{
  ...
  {
    LOGD(QString("%1:%2 \n%3:%4\n Diff %5 : %6")
             .arg(archFirst.first.name)
             .arg(archFirst.second)
             .arg(archSecond.first.name)
             .arg(archSecond.second)
             .arg(deckArchsDiff)
             .arg(deckArchsDiffPercent));
  }
