...
QString GAnalytics::Private::getUserAgent()
{
  ...
  return QString("%1/%2 (%3; %4) GAnalytics/1.0 (Qt/%5)")
      .arg(appName)
      .arg(appVersion)
      .arg(system)
      .arg(locale)
 ...
 */
GAnalytics::GAnalytics(QObject* parent) : QObject(parent), d(new Private(this))
{
}

GAnalytics::GAnalytics(const QString& trackingID, QObject* parent) : QObject(parent), d(new Private(this))
{
 ...
 * @param screenName
 */
void GAnalytics::sendAppView(const QString& screenName, const QVariantMap& customValues)
{
