  ...
  , deckLosses(0)
{
  ...
  connect(deckProfileAction, &QAction::triggered, this, [this]() {
    ...
    QString deckLink = QString("%1/user/decks/"
                               "%2?localId=%3&email=%4&idToken=%5&refreshToken=%6&userName=%7&expiresIn=%8")
                           .arg(URLs::SITE())
                           .arg(deck.id)
                           .arg(userSettings.userId)
                           .arg(userSettings.userEmail)
                           .arg(userSettings.userToken)
                           .arg(userSettings.refreshToken)
                           .arg(userSettings.getUserName())
...
void DeckOverlayPlayer::afterPaintEvent(QPainter& painter)
{
  ...
  QImage settingsScaled = settings.scaled(buttonSize, buttonSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
  ...
  QImage publishScaled = publish.scaled(buttonSize, buttonSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
  ...
  {
    eventNameWithWinRate = QString("%1 | %2-%3 (%4%)").arg(eventName).arg(deckWins).arg(deckLosses).arg(deckWinRate);
  }
...
void DeckOverlayPlayer::drawStatistics(QPainter& painter)
{
  ...
  drawText(painter, statisticsFont, statisticsPen, statisticsTitle, statisticsTitleOptions, false, statisticsTitleX,
           statisticsTitleY, statisticsTextHeight, uiWidth - statisticsBorderMargin);
  // Statistics text
  ...
  drawText(painter, statisticsFont, statisticsPen, statisticsText1, statisticsTextOptions, false, statisticsTextX,
           statisticsText1Y, statisticsTextHeight, uiWidth - statisticsBorderMargin);
  int statisticsText2Y = statisticsText1Y + statisticsTextHeight + statisticsTextMargin;
  QString statisticsText2 =
      QString("4x: %1%    Land: %2%").arg(drawChance4xCards, 0, 'g', 2).arg(drawChanceLandCards, 0, 'g', 2);
  drawText(painter, statisticsFont, statisticsPen, statisticsText2, statisticsTextOptions, false, statisticsTextX,
           statisticsText2Y, statisticsTextHeight, uiWidth - statisticsBorderMargin);
...
void DeckOverlayPlayer::onHoverMove(QHoverEvent* event)
{
  ...
  {
    showingTooltip = true;
    QString tooltip = QString(tr("Publish/Update deck as public in %1")).arg(URLs::SITE());
