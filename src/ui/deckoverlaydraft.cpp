...
QList<Card*> DeckOverlayDraft::getDeckCardsSorted()
{
  ...
  std::sort(std::begin(sortedDeckCards), std::end(sortedDeckCards), [](Card*& lhs, Card*& rhs) {
    ...
    return std::make_tuple(lhs->isBasicLand(), rarities.indexOf(lhs->rarity), lhs->isLand, colors.indexOf(lhsColors),
                           lhsDualFirstColor, lhs->name, lhs->cmc) <
           std::make_tuple(rhs->isBasicLand(), rarities.indexOf(rhs->rarity), lhs->isLand, colors.indexOf(rhsColors),
                           rhsDualFirstColor, rhs->name, rhs->cmc);
  });
...
void DeckOverlayDraft::beforeDrawCardEvent(QPainter& painter, Card* card, int cardBGY)
{
  ...
  drawText(painter, cardFont, rankPen, rank, rankTextOptions, true, rankX, rankY, rankTextHeight, rankWidth);
}
...
void DeckOverlayDraft::drawHoverCard(QPainter& painter)
{
  ...
  if (currentSource == "lsv")
  {
    desc = QString("\t\t\t\t\tChannelFireball LSV Tier: %1\n%2").arg(getHoverCardRank()).arg(hoverCard->lsvDesc);
  }
  ...
  QRect textRect(descX + rankDescTextMargin, descY, width - rankDescTextMargin * 2, height);
  QRect textShadowRect(textRect.left() + 1, textRect.top() + 1, textRect.width(), textRect.height());
