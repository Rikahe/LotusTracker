...
TabOverlay::TabOverlay(QWidget* parent) : QWidget(parent), ui(new Ui::TabOverlay)
{
  ui->setupUi(this);
  applyCurrentSettings();
  connect(ui->rbMTG, &QRadioButton::clicked, this, &TabOverlay::onCardLayoutChanged);
  connect(ui->rbMTGA, &QRadioButton::clicked, this, &TabOverlay::onCardLayoutChanged);
  connect(ui->cbShowCardOnHover, &QCheckBox::clicked, this, &TabOverlay::onShowCardOnHoverChanged);
  connect(ui->cbShowCardManaCost, &QCheckBox::clicked, this, &TabOverlay::onShowCardManaCostChanged);
  connect(ui->cbShowOnlyRemainingCard, &QCheckBox::clicked, this, &TabOverlay::onShowOnlyRemainingCardsChanged);
  connect(ui->hsAlpha, &QSlider::valueChanged, this, &TabOverlay::onTrackerAlphaChanged);
  connect(ui->hsUnhideDelay, &QSlider::valueChanged, this, &TabOverlay::onUnhideDelayChanged);
  connect(ui->cbPTStatistics, &QCheckBox::clicked, this, &TabOverlay::onPTStatisticsChanged);
}
...
void TabOverlay::applyCurrentSettings()
{
  ...
  QString unhideText = delay == 0 ? tr("(disabled)") : QString(tr("(%1 seconds)")).arg(delay);
