...
DeckOverlayBase::DeckOverlayBase(QWidget* parent)
  : QMainWindow(parent)
  , ui(new Ui::TrackerOverlay())
  , cardBGSkin(APP_SETTINGS->getCardLayout())
  , mousePressed(false)
  , mouseInitialPosition(QPoint())
  , cornerRadius(10)
  , uiPos(0, 50)
  , zoomMinusButton(QRect(0, 0, 0, 0))
  , zoomPlusButton(QRect(0, 0, 0, 0))
  , uiAlpha(1.0)
  , cardHoverWidth(220)
  , coverHeight(0)
  , uiHeight(0)
  , uiWidth(BASE_UI_WIDTH)
  , uiScale(2)
  , currentHoverPosition(0)
  , hoverCard(nullptr)
  , deck(Deck())
  , hidden(false)
...
void DeckOverlayBase::setupWindow()
{
  ...
  objc_msgSend(nsWindowObject, sel_registerName("setCollectionBehavior:"),
...
void DeckOverlayBase::drawCoverButtons(QPainter& painter)
{
  ...
  QImage zoomMinusScaled =
      zoomMinus.scaled(zoomButtonSize, zoomButtonSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
...
void DeckOverlayBase::drawDeckInfo(QPainter& painter)
{
  ...
  drawText(painter, titleFont, titlePen, deckName, titleTextOptions, true, uiPos.x(), getDeckNameYPosition(),
           titleHeight, uiWidth);
  // Deck identity
...
void DeckOverlayBase::drawDeckCards(QPainter& painter)
{
  ...
  for (Card* card : getDeckCardsSorted())
  {
    ...
    cardQtdY -= 1;
#endif
    drawText(painter, cardFont, cardTextPen, cardQtd, cardQtdOptions, false, cardQtdX, cardQtdY, cardTextHeight,
             cardQtdWidth);
    // Card mana
    ...
    drawText(painter, cardFont, cardTextPen, cardName, cardTextOptions, false, cardQtdX + cardQtdWidth, cardQtdY,
             cardTextHeight, cardNameWidth);
...
void DeckOverlayBase::drawExpandBar(QPainter& painter)
{
  ...
  QImage expandPlusScaled =
      expandPlus.scaled(expandButtonSize, expandButtonSize, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
...
void DeckOverlayBase::drawHoverCard(QPainter& painter)
{
  ...
  QString imageFile = QString("%1%2%3.png").arg(cachesDir).arg(QDir::separator()).arg(hoverCard->mtgaId);
  ...
  else
  {
    ...
    connect(reply, &QNetworkReply::finished, this, &DeckOverlayBase::onCardImageDownloaded);
  }
  ...
  QImage cardImgScaled = cardImg.scaled(cardHoverSize, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation);
...
void DeckOverlayBase::onCardImageDownloaded()
{
  ...
  cardImg.save(QString("%1%2%3.png").arg(cachesDir).arg(QDir::separator()).arg(mtgaId));
  update();
}
...
{
  if (event->pos().x() < uiPos.x() || event->pos().x() > uiPos.x() + uiWidth)
  {
    return;
  }
...
void DeckOverlayBase::mousePressEvent(QMouseEvent* event)
{
  ...
  if (zoomMinusButton.contains(event->pos()) || zoomPlusButton.contains(event->pos()))
  {
    return;
  }
  mousePressed = true;
