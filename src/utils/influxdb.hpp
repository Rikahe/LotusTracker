...
protected:
  ...
  int _send_udp(const std::string& host, int port)
  {
    ...
    if (info.error() == QHostInfo::NoError)
    {
      ...
      if (sock->writeDatagram(&lines_.str()[0], lines_.str().length(), address, static_cast<quint16>(port)) <
          lines_.str().length())
      {
        return -3;
      }
...
{
  detail::tag_or_field_caller& tag(const std::string& k, const std::string& v)
  {
    return _t(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, const std::string& v)
  {
    return _f_s(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, bool v)
  {
    return _f_b(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, short v)
  {
    return _f_i(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, int v)
  {
    return _f_i(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, long v)
  {
    return _f_i(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, long long v)
  {
    return _f_i(k, v);
  }
  detail::tag_or_field_caller& field(const std::string& k, double v, int prec = 2)
  {
    return _f_f(k, v, prec);
  }
  detail::ts_caller& timestamp(unsigned long long ts)
  {
    return _ts(static_cast<long long>(ts));
  }
