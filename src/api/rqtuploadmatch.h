...
public:
  RqtUploadMatch(MatchInfo matchInfo, Deck playerDeck, QString playerRankClass)
  {
    ...
    QString gameResult = QString("%1x%2").arg(matchInfo.playerGameWins).arg(matchInfo.playerGameLoses);
    QJsonObject player1Json{ { "arch", playerDeck.arch() },
                             { "cards", cards2JsonMapValue(playerDeck.cards(true)) },
                             { "colors", playerDeck.colorIdentity() },
                             { "deck", playerDeck.id },
                             { "name", playerDeck.name },
                             { "rank", playerRankClass } };
    QJsonObject player2Json{ { "arch", matchInfo.getOpponentDeckArch() },
    ...
    QJsonObject jsonObj{ { "event", matchInfo.eventId },
...
private:
  QJsonObject games2JsonMapValue(QList<GameInfo> games)
  {
    ...
    for (GameInfo gameInfo : games)
    {
      ...
      QJsonObject jsonGame{ { "duration", gameInfo.duration },
                            { "first", first },
