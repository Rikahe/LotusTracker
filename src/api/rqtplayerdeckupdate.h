...
private:
  ...
  QJsonObject getDiffCardsJson(QMap<Card*, int> deckCards, QMap<Card*, int> oldDeckCards)
  {
...
public:
  RqtPlayerDeckUpdate(Deck deck, Deck oldDeck)
  {
    ...
    _body = QJsonDocument(
        QJsonObject{ { "deckId", deck.id }, { "mainDeck", jsonDiffCards }, { "sideboard", jsonDiffSideboard } });
