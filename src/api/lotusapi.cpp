...
LotusTrackerAPI::LotusTrackerAPI(QObject* parent)
{
  ...
  connect(this, &LotusTrackerAPI::sgnTokenRefreshed, this, &LotusTrackerAPI::onTokenRefreshed);
}
...
void LotusTrackerAPI::refreshToken(const QString& refreshToken)
{
  ...
  request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
  QNetworkReply* reply = networkManager.post(request, body);
  connect(reply, &QNetworkReply::finished, this, &LotusTrackerAPI::tokenRefreshRequestOnFinish);
}
...
void LotusTrackerAPI::recoverPassword(const QString& email)
{
  ...
  request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
  QNetworkReply* reply = networkManager.post(request, body);
  connect(reply, &QNetworkReply::finished, this, &LotusTrackerAPI::recoverPasswordRequestOnFinish);
}
...
{
  QDateTime now = QDateTime::currentDateTime();
  if (!lastUpdatePlayerInventoryDate.isNull() && lastUpdatePlayerInventoryDate.secsTo(now) <= UPDATE_INVENTARY_INTERVAL)
  {
    return;
  }
...
void LotusTrackerAPI::uploadMatch(const MatchInfo& matchInfo, const Deck& playerDeck, QString playerRankClass)
{
  ...
  connect(reply, &QNetworkReply::finished, this, &LotusTrackerAPI::uploadMatchRequestOnFinish);
}

void LotusTrackerAPI::uploadEventResult(QString eventId, QString deckId, QString deckColors, int maxWins, int wins,
                                        int losses)
{
  UserSettings userSettings = APP_SETTINGS->getUserSettings();
  if (!userSettings.isUserLogged())
