...
void _writeLog(const QString& inSignal, const QStringList& inFrameInfoList)
{
  ...
  const QString cFileName = QStringLiteral("%1/%2 %3 Crash.log")
                                .arg(sCrashReportDirPath, QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss"),
...
#else
  const QString cCommand = QStringLiteral("%1/tools/addr2line -f -p -e %2 %3")
                               .arg(QCoreApplication::applicationDirPath(), inProgramName, cAddrStr);
#endif
...
#error You need to define the stack frame layout for this architecture
  ...
  while (StackWalk64(image, process, thread, &stackFrame, context, NULL, SymFunctionTableAccess64, SymGetModuleBase64,
                     NULL))
  {
    ...
    if (!cSymbol.isNull())
    {
      ...
      const char* cFunctionName =
          abi::__cxa_demangle(cSymbol.toLatin1().constData(), nullptr, nullptr, &demangleStatus);
...
void setSignalHandler(const QString& inCrashReportDirPath, logWrittenCallback inLogWrittenCallback)
{
  ...
  {
    sCrashReportDirPath = QStringLiteral("%1/%2 Crash Logs")
